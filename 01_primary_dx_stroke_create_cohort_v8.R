################################################################################
# SCRIPT 01: CREATE STROKE HOSPITALIZATION COHORT
...
#
# Inputs:
#   - CDW OMOP: OMOPV5_VISIT_OCCURRENCE, OMOPV5_CONDITION_OCCURRENCE
#   - CMS OMOP: OMOP_CMS_VISIT_OCCURRENCE_FF291, OMOP_CMS_CONDITION_OCCURRENCE_FF291
#   - Crosswalk tables: cw_icn, cw_scrssn
#
# Outputs:
#   - pdx_stroke_complete.parquet: All stroke hospitalizations (raw)
#   - pdx_stroke_visit_summary.parquet: Rolled-up stroke episodes
#
# Expected Results:
#   - ~1M rows (174K unique patients) in complete dataset
#   - ~192K rows (174K unique patients) in summary dataset
#
################################################################################
...
# CREATE STROKE HOSPITALIZATION COHORT\n")
...
omop_xw <- tbl(con, in_schema(schema_src, 'OMOPV5Map_SPatient_PERSON')) %>% 
  inner_join(cw_icn, by = "PatientICN")
...
pdx_stroke_complete <- rbind(omop_pdx_inpatient_cdw, omop_pdx_inpatient_cms)
log_count(pdx_stroke_complete, "Combined inpatient visits (CDW + CMS)")

# Save raw combined data
save_parquet_safe(pdx_stroke_complete, "pdx_stroke_complete.parquet")

# Open parquet:
pdx_stroke_complete <- open_dataset("parquet/pdx_stroke_complete.parquet") %>% collect()
...
pdx_stroke_complete_rollup <- pdx_stroke_complete %>% 
...
log_count(pdx_stroke_complete_rollup, "Visits with rollup indicators")
...
last_stroke_icd <- pdx_stroke_complete_rollup %>% 
  filter(pdx_stroke == 1) %>% 
...
pdx_stroke_visit_summary <- pdx_stroke_complete_rollup %>% 
  group_by(PatientICN, admit_run) %>% 
  summarize(
...
log_count(pdx_stroke_visit_summary, "Final stroke hospitalizations")
...
save_parquet_safe(pdx_stroke_visit_summary, "pdx_stroke_visit_summary.parquet")
